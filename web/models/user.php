	...
	class User extends person {
		...
		public function __construct() {
			...
			if(func_num_args() == 1) {
				...
				$command->execute();
				//bind results
				$command->bind_result($username, $lastLogin, $firstName, $lastName, $birthdate, $gender, $photo, $email, $phoneNumber);
				//fetch data
				$found = $command->fetch();
				//close command
				...
				if ($found) {
					...
					$this->gender = $gender;
					$this->photo = $photo;
					$this->contactData = new ContactData($phoneNumber, $email);
				}
				else {
		...
		public static function getAll() {
			...
			$connection = MySqlConnection::getConnection();
			//query
			$query = 'call usp_getAllUsers()';
			//command
			$command = $connection->prepare($query);
			//execute
			$command->execute();
			//bind results
			$command->bind_result($username, $lastLogin, $firstName, $lastName, $birthdate, $gender, $photo, $email, $phone);
			//fetch data
			while ($command->fetch()) {
				$contactData = new ContactData($phone, $email);
				$account = new Account($username, $lastLogin);
				array_push($list, new User($account, $firstName, $lastName, $birthdate, $gender, $photo, $contactData));
			}
			//close command
			mysqli_stmt_close($command);
			//close connection
